...
# --- INTERFAZ PROFESIONAL SIN REGISTRO ---
UI = f"""
...
<style>
    ...
    #log {{ flex:1; overflow-y:auto; background:rgba(0,0,0,0.5); padding:20px; border:1px solid #00f2ff11; font-family:monospace; font-size:13px; line-height:1.6; }}
...
<body>
    <div class="sidebar">
        <h2 style="margin:0; letter-spacing:2px;">SENTINEL CORE</h2>
        <div class="card">
            <strong>FINANCIAL INTEL</strong><br><small style="color:#888;">Expert Market Analysis</small>
            <button class="btn" onclick="quick('Analiza tendencias de mercado DeFi y Solana')">MARKET SCAN</button>
        </div>
        <div class="card">
            <strong>MEDICAL INTEL</strong><br><small style="color:#888;">Diagnostic Support AI</small>
            <button class="btn" onclick="quick('Actúa como asistente médico experto para análisis clínico')">MEDICAL SCAN</button>
        </div>
        <div class="card" style="border-color:#27e19d55;">
            <strong style="color:#27e19d;">GATEWAY SOLANA USDC</strong>
            <div class="usdc-box">ADDRESS:<br><b>{Config.W_ADDR}</b></div>
    ...
    <div class="main">
        <div id="log">>> SISTEMA EN LÍNEA... CONOCIMIENTO MÉDICO Y FINANCIERO DESBLOQUEADO.<br>>> SINCRONIZADO CON RED SOLANA.</div>
        <input type="text" id="in" style="width:100%; padding:18px; background:#000; border:1px solid #00f2ff; color:#fff; margin-top:20px; outline:none; font-size:16px;" 
               placeholder="Escriba su consulta técnica..." onkeydown="if(event.key==='Enter') send()">
    </div>
    <script>
        async function send() {{
            ...
            l.innerHTML += `<div style="color:#00f2ff88; margin-top:15px;">> USER: ${{m}}</div>`;
            ...
            l.innerHTML += `<div style="color:#fff; padding:10px; border-left:2px solid #00f2ff; background:rgba(255,255,255,0.03);">> CORE: ${{d.response}}</div>`;
...
def quantum_core_engine():
    try:
        ...
        sys_msg = "Eres SENTINEL CORE v3.0, experto en medicina y finanzas. Responde con lenguaje técnico y profesional."
